...
public class BundleInfoRepository {
    ...
    public List<Map<String, Object>> findBills(String zgrpno) {
        String sql = "SELECT RECP_TP_TX, ORDER_NO, VTEXT, GOODS_CD, INST_DT, USE_DUTY_MONTH, OWNER_DATE, USE_MONTH, RECP_YM, FIX_SUPPLY_VALUE, FIX_VAT, FIX_BILL_AMT, SUPPLY_VALUE, VAT, BILL_AMT, PAY_COM_TX, PAY_NO, INST_JUSO, GOODS_SN, DEPT_CD_TX, DEPT_TELNR, ZBIGO, GOODS_TX " +
                "FROM z_re_b2b_bill_info WHERE ZGRPNO = :zgrpno";
        List<Object[]> rows = em.createNativeQuery(sql)
                .setParameter("zgrpno", zgrpno)
                .getResultList();
        String[] keys = {"RECP_TP_TX", "ORDER_NO", "VTEXT", "GOODS_CD", "INST_DT", "USE_DUTY_MONTH", "OWNER_DATE", "USE_MONTH", "RECP_YM", "FIX_SUPPLY_VALUE", "FIX_VAT", "FIX_BILL_AMT", "SUPPLY_VALUE", "VAT", "BILL_AMT", "PAY_COM_TX", "PAY_NO", "INST_JUSO", "GOODS_SN", "DEPT_CD_TX", "DEPT_TELNR", "ZBIGO", "GOODS_TX"};
